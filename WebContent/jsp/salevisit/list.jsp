...
<SCRIPT language=javascript>
	...
	$(function(){
		...
		$('.menu_time').datepick({dateFormat: 'yy-mm-dd'}); 
	});
</SCRIPT>
										...
										<TABLE cellSpacing=0 cellPadding=2 border=0>
											<TBODY>
												<TR>
													<TD>拜访时间：</TD>
													<TD>
													<INPUT class=menu_time id="visit_time"
														style="WIDTH: 80px" maxLength=50 name="visit_time" value="<s:date name="visit_time" format="yyyy-MM-dd" />" readonly="readonly" 
														value="">
													</TD>
													<TD>至</TD>
													<TD>
													<INPUT class=menu_time id="visit_end_time"
														style="WIDTH: 80px" maxLength=50 name="visit_end_time" value="<s:date name="visit_end_time" format="yyyy-MM-dd" />" readonly="readonly" 
														value="">
													</TD>
								...
								<TR>
									<TD><SPAN id=pagelink>
											<DIV
												...
												共[<B>${totalCount}</B>]条记录,共[<B>${totalPage}</B>]页
												,每页显示
												...
												条
												<s:if test="currPage!=1">[<A href="javascript:to_page(${currPage-1})">前一页</A>]</s:if>
												<B>${currPage}</B>
												<s:if test="currPage!=totalPage&&totalPage!=1">[<A href="javascript:to_page(${currPage+1})">后一页</A>]</s:if>
												到
												<input type="text" size="3" id="page" name="currPage" value="${currPage}" />
												页
